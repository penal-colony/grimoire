 ...
 *
 * Multi-lorebook support (v2):
 * - Lorebook metadata stored under jlb_lorebooks
 * - Per-lorebook entries stored under jlb_entries_<lorebookId>
 * - getAllEnabledEntries() flattens enabled lorebooks into a single entry array
 */
...
const Lorebook = {
  ...
  parseSTFormat(json) {
    ...
    // Also handle { entries: [...] } array variant
  ...
  /**
   * Create a new lorebook metadata object.
   * @param {string} name - Lorebook name
   * @param {string} description - Optional description
   * @returns {object} Lorebook metadata
   */
  ...
  /**
   * Flatten all enabled lorebooks' entries into a single array.
   * Used by content.js to send combined entries to inject.js.
   *
   * @param {Array} lorebooks - Array of lorebook metadata objects
   * @param {object} entriesByLorebook - Map of lorebookId -> entries array
   * @returns {Array} Combined entry array from all enabled lorebooks
   */
  getAllEnabledEntries(lorebooks, entriesByLorebook) {
    if (!Array.isArray(lorebooks) || !entriesByLorebook || typeof entriesByLorebook !== "object") {
      return [];
    }
